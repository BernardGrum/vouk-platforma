...
function RegistracijaKorak1() {
  ...
  useEffect(() => {
    const jeEmailValidan = email.includes('@') && email.length > 5;
    const jeLozinkaValidna = lozinka.length >= 8;
    if (jeEmailValidan && jeLozinkaValidna && uvjeti) {
      setFormaJeValidna(true);
    } else {
      setFormaJeValidna(false);
    }
  }, [email, lozinka, uvjeti]);
  ...
  return (
    <div className="h-screen w-screen bg-neutral-100 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg border border-neutral-200 w-full max-w-md">
        ...
        {/* Celotna forma je sedaj en blok */}
        <div className="space-y-4">
            {/* Polja za vnos */}
            <div>
                <label htmlFor="email" className="block text-sm font-semibold text-neutral-700 mb-2">E-pošta</label>
                <input type="email" id="email" className="block w-full border border-neutral-300 rounded-md shadow-sm p-2.5 focus:ring-2 focus:ring-primary-500 transition" placeholder="npr. ime.prezime@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div>
                <label htmlFor="password" className="block text-sm font-semibold text-neutral-700 mb-2">Lozinka</label>
                <div className="relative">
                <input type={prikaziLozinku ? 'text' : 'password'} id="password" className="block w-full border border-neutral-300 rounded-md shadow-sm p-2.5 focus:ring-2 focus:ring-primary-500 transition" placeholder="Minimalno 8 znakova" value={lozinka} onChange={(e) => setLozinka(e.target.value)} />
            ...
            <div>
                ...
                <input type="checkbox" className="h-4 w-4 rounded border-neutral-300 text-primary-600 focus:ring-primary-500" checked={uvjeti} onChange={(e) => setUvjeti(e.target.checked)} />
             ...
             {/* POPRAVEK: Gumb je sedaj stiliziran kot 'secondary' in je del forme */}
            <Button onClick={handleNextStep} variant="secondary" className="w-full" disabled={!formaJeValidna}>
